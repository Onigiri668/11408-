...
void BFS_MIN_Disrance(Graph G,int u){
    for(int i=0; i<G.vexnum; i++){		//初始化数组
  	...
  	for(int i=0; i<G.vexnum; i++){
        ...
        // 检查所有邻接⾃v的顶点路径长度是否最短
        for(int j=0; j<G.vexnum; j++){
	        if(final[j]!=TRUE && dist[j]>dist[v]+G.edge[v][j]){
...
- **平衡二又树AVL**：插入/删除 很容易破坏“平衡”特性，需要频繁调整树的形态。如:插入操作导致平衡，则**需要先计算平衡因子，找到最小不平衡子树 (时间开销大)**，再进行LL/RR/LR/RL调整；

- **红黑树 RBT**：插入/删除 很多时候不会破坏“红黑”特性，无需频繁调整树的形态。即便需要调整，**一般都可以在常数级时间内完成**；>

  > **平衡二叉树**：适用于以查为主、很少插入/删除的场景；
  > **红黑树**：适用于频繁插 入、删除的场景，实用性更强。
